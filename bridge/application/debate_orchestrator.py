...
class DebateOrchestrator:
    ...
    def _analyze_consensus(
        ...
        Args:
            ...
            user_lang: User language code for i18n output.

        Returns:
            Brief consensus/dissent assessment.
        """
        if len(responses) < 2:
            return t("debate.only_one_provider", user_lang)
        ...
        if avg_overlap > 0.35:
            return t("debate.consensus_high", user_lang, overlap=f"{avg_overlap:.0%}")
        elif avg_overlap > 0.20:
            return t(
                "debate.consensus_partial", user_lang, overlap=f"{avg_overlap:.0%}"
            )
        else:
            return t("debate.consensus_low", user_lang, overlap=f"{avg_overlap:.0%}")
    ...
    def _build_judge_prompt(
        ...
        Args:
            ...
            responses: Provider name -> response text.
            user_lang: User language code (synthesis/recommendation in this language).
    ...
    async def final_review(
        ...
        prompt, label_to_provider = self._build_judge_prompt(
