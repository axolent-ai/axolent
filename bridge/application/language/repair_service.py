...
class RepairService:
    ...
    async def repair(
        ...
        # Guard: output too long for full rewrite
        if len(original_output) > _MAX_REPAIR_OUTPUT_LENGTH:
            log.info(
                "RepairService: output too long (%d chars > %d), skipping full rewrite",
                len(original_output),
    ...
    def build_audit_entry(self, result: RepairResult) -> dict[str, Any]:
        ...
        entry: dict[str, Any] = {
            "event_type": (
                "language_repair_succeeded"
                if result.was_repaired
                else (
                    "language_repair_failed"
                    if result.repair_failed
                    else "language_repair_skipped"
                )
            ),
